# -*- coding: utf-8 -*-
"""
Created on Thu Mar 27 10:42:27 2025
...
class DroneWithBorderGradient:
    ...
    def loop_once(self):
        ...
        # ### If in a special mode => display that screen
            ...
            for k in keyups:
                if k== ord('q'):
                    ...
                    if self.special_mode==1:
                        # user pressed E => run #3
                        self.special_mode=0
                        self.current_env_index=2
                        self.reset_env(load_new_env=True)
                        print("[INFO] Moved to environment #3")
                    elif self.special_mode==2:
                        # user pressed E => that means we proceed to run #9 => index=8
                        self.special_mode=0
                        self.current_env_index=8
                        self.reset_env(load_new_env=True)
                        print("[INFO] Moved to environment #9, no guidance")
            self.draw_special_mode_screen()
            return
        ...
        for k in keyups:
            if k==ord('q'):
                ...
                if self.current_env_index==1:
                    self.special_mode=1
                    print("[INFO] special_mode=1 => user must press E to continue")
                elif self.current_env_index==7:
                    self.special_mode=2
                    print("[INFO] special_mode=2 => user must press E => no guidance for runs 9..10")
                elif self.current_env_index< self.TOTAL_RUNS -1:
        ...
        if run>=3 and run<=8:
            gradient_factor=0.6
            attraction_factor=0.6
            
            # compute gradient if self.gradient_on==True => multiply by factor
            if self.gradient_on and self.state=="RUNNING":
                f_grad= self.compute_gradient_force() * gradient_factor
            else:
                f_grad=np.array([0,0],dtype=float)
                    
            if self.attraction_on and self.state=="RUNNING":
                f_attraction= self.compute_attractive_force() * attraction_factor
            else:
                f_attraction=np.array([0,0],dtype=float)
    ...
    def get_run_guidance(self):
        """
        Return (gradient_factor, attraction_factor) for the current study & run.
        Study 1 => 0% always
        Study 2 => run=1,2 =>0% => run=3..8 =>60% => run=9,10 =>0
        Study 3 => run=1,2 =>0 => run=3..9 => linearly 1.0 =>0 => run10 =>0
        """
        s= self.current_study
        r= self.current_run
        if s==1:
            # always 0
            return (0.0,0.0)
            ...
            if r in [1,2,10]:
                ...
                # linearly from run3=100% => run9=0% => total 7 steps (3..9)
                # run3 => 1.0, run4 => ~0.83..., run5 =>0.66..., run6=>0.5..., run7=>0.33..., run8=>0.16..., run9=>0
                # let's define "frac" from 0..1 => run3= 0 => run9=1 => but we want to invert it
                step= (r-3) # 0..6
                ratio= step/6.0  # 0..1
                # we want 1.0 => 0 => so factor= 1.0 - ratio
                factor= 1.0- ratio
